...
public class SCTPSocket {
	...
	private long impl;
	SCTPSocket(boolean one_to_many) throws SocketException {
		impl=0;
		open(one_to_many);
	}
	SCTPSocket(boolean one_to_many, int port) throws SocketException {
		impl=0;
		open(one_to_many);
		bind(port);
	}
	...
	* @deprecated An important feature of SCTP is multi-homing. This method binds the socket to a single interface and effectively disables multi-homing. This method is only here for completeness' sake.
	 ...
	 * note: This method waits for any otustanding receive() calls to finish.
	 */
	...
	*/
	 ...
	 *Until listen() has been called inbound associations are not accepted.
	 */
